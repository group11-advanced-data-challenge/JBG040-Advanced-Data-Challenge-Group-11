...
# Random import for reproducibility (NEW)
import random
...
# ====== NEW: Ensuring reproducible results =====
...
def main(args: argparse.Namespace, activeloop: bool = True) -> None:
    ...
    # save plot of losses
    fig.savefig(
        Path("artifacts") / f"session_{now.month:02}_{now.day:02}_{now.hour}_{now.minute:02}_loss.png"
    )
    ...
    ax_cm.set_title("Confusion Matrix (Baseline)")
    fig_cm.tight_layout()
    fig_cm.savefig(
        Path("artifacts") / f"session_{now.month:02}_{now.day:02}_{now.hour}_{now.minute:02}_cm.png"
    )

    # Save text metrics
    metrics_path = Path("artifacts") / f"session_{now.month:02}_{now.day:02}_{now.hour}_{now.minute:02}_metrics.txt"
